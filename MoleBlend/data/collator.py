...
def collator_3d(items, max_node=512, multi_hop_max_dist=20, spatial_pos_max=20):
    ...
    weights = None
    if not hasattr(items[0], 'train_mean'):
        items = [(item.idx, item.attn_bias, item.attn_edge_type, item.spatial_pos, item.in_degree,
        ...
        if hasattr(items[0], 'weight'):
            items = [(item.idx, item.attn_bias, item.attn_edge_type, item.spatial_pos, item.in_degree,
                item.out_degree, item.x, item.edge_input[:, :, :multi_hop_max_dist, :], item.y, item.pos,
                item.train_mean, item.train_std, item.type, torch.from_numpy(item.weight)

                # spd_count, full_path_information
                # item.spatial_pos_count, item.node_input[:, :, :multi_hop_max_dist+1, :]
                ) for item in items]
            idxs, attn_biases, attn_edge_types, spatial_poses, in_degrees, out_degrees, xs, edge_inputs, ys, poses, means, stds, types, weights = zip(*items)
        else:
            items = [(item.idx, item.attn_bias, item.attn_edge_type, item.spatial_pos, item.in_degree,
                item.out_degree, item.x, item.edge_input[:, :, :multi_hop_max_dist, :], item.y, item.pos,
    ...
    if ys[0].shape[0] > 1:
        ys = [item.reshape(1, -1) for item in ys]
    y = torch.cat(ys)
    x = torch.cat([pad_2d_unsqueeze(i, max_node_num) for i in xs])
    edge_input = torch.cat([pad_3d_unsqueeze(
        i, max_node_num, max_node_num, max_dist) for i in edge_inputs])
