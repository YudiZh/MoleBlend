...
class GraphormerPYGDataset(Dataset):
    ...
    def __init__(
        ...
        if train_idx is None and train_set is None:
            self.atomref = self.dataset.atomref(self.task_idx)
    ...
    def __getitem__(self, idx):
        if isinstance(idx, int):
            ...
            item.idx = idx
            if self.data_name is not None and 'molecule' in self.data_name:
                item.y = item.y.reshape(-1)
            else:
                item.y = item.y.reshape(-1)[self.task_idx].unsqueeze(0)
            if self.atomref is not None:
                item.y = self.get_energy_data(item)
    ...
    def get_energy_data(self, item):
        atomic_num = item.x[:,0] + 1 # feature index to atmoic_num
        atomref_energy = self.atomref.squeeze()[atomic_num].sum()
        return (item.y.squeeze() - atomref_energy).unsqueeze(dim=0)
    
    def get_atomref(self):
        return self.dataset.atomref(self.task_idx)
