...
def smiles2graph(smiles_string):
    """
    Following OGB: https://github.com/snap-stanford/ogb/blob/a47b716f7e972f666eae9909ee0f922cd0f9d966/ogb/utils/mol.py#L6
...
def smiles2graph_cut(smiles_string, remove_Hs=True):
    """
    Following OGB: https://github.com/snap-stanford/ogb/blob/a47b716f7e972f666eae9909ee0f922cd0f9d966/ogb/utils/mol.py#L6
    Converts SMILES string to graph Data object
    :input: SMILES string (str)
    ...
    try:
        ...
        for atom in mol.GetAtoms():
            atom_features_list.append(atom_to_feature_vector(atom)) 
        atom_features_list = atom_features_list[:cut_length]
        x = np.array(atom_features_list, dtype = np.int64)
        ...
        if len(mol.GetBonds()) > 0: # mol has bonds
            ...
            for bond in mol.GetBonds():
                ...
                if i >= cut_length or j>= cut_length:
                    continue
...
class PygPCQM4Mv2PosDataset(InMemoryDataset):
    ...
    def process(self):
        # [4M, 3], the 2nd dim has form: (idx, smiles, hl-gap), some of hl-gap are nan
        # e.g., 5, CCN(C(=O)C)/C=C/c1ccccc1Cl, 4.47899
        data_df = pd.read_csv(osp.join(self.raw_dir, 'data.csv.gz'))
        graph_pos_list = Chem.SDMolSupplier(osp.join(self.original_root, 'pcqm4m-v2-train.sdf'))  # 4M mols
        smiles_list = data_df['smiles']
        homolumogap_list = data_df['homolumogap']
